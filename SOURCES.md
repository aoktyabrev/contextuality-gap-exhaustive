...
## S14 — база квантовых графов НАЙДЕНА; §S11 отменяется в части недоступности

**§S11 не правится.** Он верно описывает состояние на 2026-08-20: адрес мёртв, данные не
архивированы, пографовая сверка невыполнима. Ниже — что изменилось.

**Письмо.** 2026-08-26 отправлен запрос Ларсу Эйрику Даниэльсену на `larsed@gmail.com`
(адрес взят с его собственной страницы `codetables.de/larsed/`, а не по памяти).
**2026-08-27 07:33 UTC получен ответ:**

> Hi,
>
> Does this page have the data you need?: https://codetables.de/larsed/quantum_graphs/
>
> --Lars Eirik

**Выгрузка, обращение 2026-08-27.** `sources/quantum_graphs/`, контрольные суммы в
`sources/quantum_graphs.sha256`:

| файл | байт | sha256 (первые 16) |
|---|--:|---|
| `index.html` | 1 671 | `ab66b2750a6577b7` |
| `quantum5` | 48 | `07c6c1a58e50e9a3` |
| `quantum6` | 147 | `c59bf9585a711082` |
| `quantum7` | 1 680 | `c1365f7913aab1c4` |
| `quantum8` | 26 424 | `0af9d78dd1e00c5f` |
| `quantum9` | 923 399 | `428d95d7f3246 7b8` |
| `quantum10.bz2` | 5 251 409 | `2aa82b149103dd5c` |

Строк: 1, 3, 33, 498, 16 533, 975 330 — ровно счётчики индексной страницы, процитированные
в §S11 по архивному снимку. Индексная страница на codetables совпадает по содержанию с
архивным снимком 2022 года.

Формат колонок (с самой страницы): `id# / nauty-string / α / ϑ / α* / d / upper bound for
orthogonal rank / Q / S / intersection number / intersection number of the complement graph
/ chromatic number of the complement graph`.

### S14.1 — пографовая сверка, которая в стадии 3 была объявлена невыполнимой

`runners/run_db_compare.py` → `results/report_db_compare.json`. Два независимых перебора
с разрывом в четырнадцать лет, сопоставленные **поимённо**, а не по счётчикам:

| n | база | наших (τ = 10⁻⁶) | только у них | только у нас | расхождений α | макс \|Δϑ\| |
|--:|--:|--:|--:|--:|--:|--:|
| 5 | 1 | 1 | 0 | 0 | 0 | 3.2·10⁻⁵ |
| 6 | 3 | 3 | 0 | 0 | 0 | 3.2·10⁻⁵ |
| 7 | 33 | 33 | 0 | 0 | 0 | 4.2·10⁻⁵ |
| 8 | 498 | 498 | 0 | 0 | 0 | 4.8·10⁻⁵ |
| 9 | 16 533 | 16 533 | 0 | 0 | 0 | 1.3·10⁻⁴ |
| 10 | 975 330 | 975 338 | **0** | **8** | 0 | 5.3·10⁻⁵ |

**Разметка graph6 совпадает без единого исключения**: каждый код базы найден у нас как
строка, без изоморфных сверок. Расхождений по числу независимости — **ноль на 992 398
графах**. Расхождения по ϑ во всех случаях укладываются в четыре печатаемых знака базы.

**При τ = 10⁻⁷ у нас появляются лишние графы на n = 8 и 9** — 2 и 67 штук. Все они
закрыты сэндвичем: χ(Ḡ) = α, то есть Δ = 0 **доказано**, а наши 1.0–1.3·10⁻⁷ есть шум
солвера. База права, мы правы, порог 10⁻⁶ — правильный.

### S14.2 — восемь графов при n = 10: гипотеза о пороге ОПРОВЕРГНУТА

Расхождение в восемь графов не объясняется порогом. Их зазоры лежат от 2.7·10⁻³ до
8.3·10⁻³ — на три порядка выше шума, — и **ни один не закрывается сэндвичем**: χ(Ḡ)
строго больше α у всех восьми.

Каждому построен **точный примальный сертификат** (`runners/certify_db_extras.py` →
`results/report_db_extras_certified.json`): рациональная матрица X, симметричная, со следом
1, с нулями на всех рёбрах, PSD по двум независимым методам, и 1ᵀX1 > α. Отсюда ϑ ≥ 1ᵀX1 > α
в точной рациональной арифметике:

| graph6 | α | доказано 1ᵀX1 ≥ | доказанный зазор ≥ |
|---|--:|---|---|
| `I?\`e\`qqUo` | 4 | 4.002735945 | 2.736·10⁻³ |
| `I?\`e\`qsZO` | 4 | 4.003703963 | 3.704·10⁻³ |
| `I?\`e\`qskg` | 4 | 4.008309934 | 8.310·10⁻³ |
| `I?\`edd[FW` | 4 | 4.007417926 | 7.418·10⁻³ |
| `I?bebOxNG` | 4 | 4.003371966 | 3.372·10⁻³ |
| `ICQe\`pcMg` | 4 | 4.006417923 | 6.418·10⁻³ |
| `ICQe\`psMg` | 4 | 4.006361924 | 6.362·10⁻³ |
| `ICvdq~sNo` | 3 | 3.002891971 | 2.892·10⁻³ |

**Вывод, сформулированный осторожно.** Восемь графов имеют ϑ > α, и это доказано. Их нет в
базе. Почему — мы не знаем и не утверждаем: причина может лежать в их перечислении, в их
фильтре, в порче файла за четырнадцать лет или в чём-то ещё, чего мы не видим. Мы
устанавливаем факт расхождения и сторону, на которой лежит доказательство, и не
приписываем причину.

---

## S15 — ревизия прав на выгрузки, 2026-08-27

Проверено по каждому файлу в `sources/`, какая лицензия объявлена и допускает ли она
хранение копии в **публичном** репозитории. Часть выгрузок изъята. **Правило 0 от этого
не страдает:** цитаты с номерами строк выше и SHA-256 ниже вместе доказывают, что
источник был прочитан именно в этом виде, и позволяют любому получить тот же файл и
проверить те же строки — не требуя, чтобы мы его перераспространяли.

### S15.1 — изъято: arXiv, лицензии без права повторного распространения

Формулировка самого arXiv про `arXiv.org perpetual, non-exclusive license 1.0`
(https://info.arxiv.org/help/license/index.html, обращение 2026-08-27):

> This license gives limited rights to arXiv to distribute the article, and also limits
> re-use of any type from other entities or individuals.

| файл | работа | лицензия на abs-странице | SHA-256 |
|---|---|---|---|
| `nie_ranestad_sturmfels_sdp_degree.pdf` | arXiv:math/0611562 | `nonexclusive-distrib/1.0` | `4be08961ee399eed9543fab10a0616418469f63e170dd493bf8af981c064293f` |
| `nie_ranestad_sturmfels_sdp_degree.txt` | извлечение из него | — | `cf3fe2ff500cf63562203eec0832156017e201f16fef61f945b1781a24105734` |
| `knuth_sandwich_theorem.pdf` | arXiv:math/9312214 | `assumed-1991-2003` | `b0ac4c3f6431641a2ff8a3e614c71cfb5038c4654109f7aa04fd6a29272c99fa` |
| `knuth_sandwich_theorem.txt` | извлечение из него | — | `2559019eeb7feb27c57dd16c17b1b1a77a6c97901fb31bfd1880a19d97f3dcd0` |
| `eprint_1211.5825` | arXiv:1211.5825 | `nonexclusive-distrib/1.0` | `2a80e5cfd8754536426e2d1293043a6415cac500b8976a0f4657e11fd7e247e5` |
| `ep1211/Fundamental_structures_17_afterpraproofs.tex` | то же | — | `d82c41ed5609b2006c68206b3cca1b52c2f27fffb47e1d772e6a1fe9642a9b7c` |
| `ep1211/Fig0.eps` | то же | — | `10447d941e4a0725f374578eb206279a3e42dd4775b2ee930dcb4f9bae4b69bc` |
| `ep1211/Fig1.eps` | то же | — | `e3371cf052e5362ce53c6569a035f3c7875815dd5aa0483ceef7cab548046619` |

**Все процитированные выше строки остаются проверяемыми:** скачать по указанному arXiv-ID,
сверить SHA-256, открыть названную строку.

### S15.2 — изъято: не цитируется вовсе

`letchford_giandomenico_rossi_theta_survey.pdf`
(`997d5d96e3fcb45ad9326124b625fe1825f3c9e839dec186d89e57731214083a`) и
`letchford_theta_survey.txt`
(`186e2848aa97f01db3a6f93fcae81893d047f486c3d659b85c9b16b0bd026e1b`) не цитируются ни в
одном отчёте и ни в одном документе кампании. Выгрузка была сделана и не пригодилась;
держать чужой обзор без надобности незачем.

### S15.3 — изъято: копия чужого кода без лицензии

`sources/quadc5_authors_repo/` — клон https://github.com/ugurtamerphys/quad-c5-contextuality
(§S2). У репозитория **не объявлено лицензии** (`gh api ... --jq .license` → `null`,
обращение 2026-08-27), то есть по умолчанию все права защищены.

**Содержательной потери нет.** Их же архив на Zenodo,
DOI 10.5281/zenodo.20465134, объявлен **CC BY 4.0** (`sources/zenodo_20465134.json`,
поле `license: {"id": "cc-by-4.0"}`), и §S2.1 фиксирует, что архив **побайтово совпадает**
с клоном. Остаётся лицензированная копия `sources/zenodo_extract/`, с атрибуцией.
Проверка `diff -rq` двух каталогов на 2026-08-27: расхождений нет.

### S15.4 — изъято: база квантовых графов, заявления о правах нет

`sources/quantum_graphs/` (§S14). На странице `codetables.de/larsed/quantum_graphs/`
нет ни лицензии, ни заявления о правах, поэтому копия не перераспространяется.
Контрольные суммы всех семи файлов остаются в `sources/quantum_graphs.sha256`, а
`runners/run_db_compare.py` скачивает их сам и **сверяет с этими суммами**, отказываясь
работать при расхождении. Читатель получает те же байты, что использовали мы, не получая
их от нас.

Предложение задепонировать базу на Zenodo под именем Даниэльсена и авторов 2012 года
отправлено 2026-08-27 и ждёт ответа; если оно будет принято, у базы появится DOI и
архивная копия, и вопрос закроется по существу, а не обходом.

### S15.5 — оставлено, с указанием условий

| источник | условия | основание |
|---|---|---|
| arXiv:2605.12828 и его LaTeX | **CC BY 4.0** | указано на abs-странице, выгрузка `sources/arxiv_2605.12828_abs.html` |
| архив авторов на Zenodo | **CC BY 4.0** | `sources/zenodo_20465134.json` |
| nauty 2.9.3 | **Apache 2.0** | `COPYRIGHT` и `LICENSE-2.0.txt` внутри тарбола |
| базы графов Маккея | ограничений на странице не заявлено; файлы читаются раннерами 0.a, 0.c, 1.a | https://users.cecs.anu.edu.au/~bdm/data/ |
| OEIS A001349 | **CC BY-SA 4.0** | их EULA, п. 2, обращение 2026-08-27 |

---

### S15.6 — цитаты, которые жили только в изъятых дампах

Ниже дословно перенесено то, на что опираются документы кампании и что до ревизии
хранилось лишь внутри удалённых файлов. Теперь оно здесь, с номерами строк, и остаётся
проверяемым по SHA-256 из §S15.1.

**Библиографическая ссылка на Ловаса**, `knuth_sandwich_theorem.txt`, запись [12]:

> `[12] L. Lov´ asz, “On the Shannon capacity of a graph,” IEEE Transactions on Information`
> `Theory IT–25 (1979), 1–7.`

Отсюда берётся цитирование Ловаса 1979 — из библиографии Кнута, а не по памяти.

**Индексная страница базы квантовых графов**, `quantum_graphs/index.html`, описание
формата (уже процитировано в §S14, повторено здесь ради самодостаточности пункта):

> `id# nauty-string α ϑ α * d upper bound for orthogonal rank`

Слово `nauty-string` — основание утверждения, что разметка базы получена
инструментом Маккея, а значит совпадение с нашими кодами проверяет **вычисление, а не
перечисление**: общий пропуск был бы невидим обеим сторонам.

### S15.7 — почему история НЕ переписывается, и что вместо этого

Изъятие из индекса сделано. Изъятие **из истории** рассмотрено и отклонено, и вот на
каких основаниях — все проверены, а не приняты.

1. **Копия кода авторов вошла коммитом `dc07d1f` — тем самым, которым запечатана
   предрегистрация стадии 0.** Чистка потребовала бы переписать 104 коммита из 105 и
   сменить SHA у всех четырёх тегов.
2. **Это разорвало бы цепочку печатей.** `scripts/verify_seals.sh` проверяет печати
   против коммитов, названных по SHA; `dc07d1f` назван и в `REPORT.md`, и в самой
   `PREREGISTRATION.md`. Переписывание сменило бы эти SHA и обесценило бы ровно то
   доказательство порядка «предсказание раньше данных», на котором стоит вся кампания.
3. **И всё равно не достигло бы цели.** Архивы на Zenodo — снимки, они неизменяемы: в
   `contextuality-gap-exhaustive-v2.2.zip` изъятые файлы присутствуют (22 записи), и то
   же верно для v1.0.0, v2.0 и v2.1. Переписывание истории на GitHub их оттуда не убрало
   бы.

Обмен получился бы такой: полная потеря аудиторского следа предрегистраций ради
**частичного** устранения копий, которые остаются в четырёх архивах с DOI. Это плохой
обмен, и он не делается.

**Что сделано вместо.** Копии убраны из текущего дерева и из будущих архивов; каждая
названа здесь с лицензией, SHA-256 и способом получить тот же файл у правообладателя;
раннер сверки скачивает базу сам и проверяет суммы. Если правообладатель попросит,
переписывание истории остаётся доступным действием — но тогда это будет осознанная плата
за конкретное требование, а не превентивная порча собственного доказательства.
